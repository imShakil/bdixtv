...
export default function ChannelApp() {
  ...
  const loadAllChannels = useCallback(async (background = false) => {
    ...
    const playlistChannels = await loadPlaylistChannels();

    const elapsed = Math.round(performance.now() - startedAt);
    recordMetric('channels_load_ms', elapsed, { background });

    if (!playlistChannels.length) {
      setStatus('error');
      logEvent('channels_load_failed', { elapsed });
      return;
    }
      ...
      if (cached) {
        try {
          ...
          if (freshCache && Array.isArray(parsed.channels) && parsed.channels.length) {
            ...
            setStatus('ready');
            logEvent('channels_cache_hydrated', { total: parsed.channels.length });
          }
        } catch {
